...
{}
Colorf PBRPointLight::sample(const Point &p, const std::array<float, 2> &lsample,
...
Colorf PBRPointLight::power(const Scene &scene) const {
	return 4 * PI * intensity;
}
bool PBRPointLight::delta_light() const {
	return true;
}
float PBRPointLight::pdf(const Point &p, const Vector &wi) const {
	return 0;
}
