...
DifferentialGeometry::DifferentialGeometry(const Point &point, const Vector &dp_du, const Vector &dp_dv,
	...
	: point(point), normal(normal), geom_normal(geom_normal), node(node), hit_side(hit_side),
	dp_du(dp_du), dp_dv(dp_dv), dn_du(dn_du), dn_dv(dn_dv), u(u), v(v), du_dx(0), dv_dx(0), du_dy(0), dv_dy(0)
{}
