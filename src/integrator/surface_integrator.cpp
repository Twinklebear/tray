	...
	const Scene &scene, Sampler &sampler, MemoryPool &pool)
{
	...
	float pdf_val = 0;
	int n_samples = 6;
	auto u_sample = pool.alloc_array<std::array<float, 2>>(n_samples);
	auto c_sample = pool.alloc_array<float>(n_samples);
	...
	for (int i = 0; i < n_samples; ++i){
		Colorf f = bsdf.sample(w_o, w_i, u_sample[i], c_sample[i], pdf_val,
